...
CREATE TABLE IF NOT EXISTS users (
  ...
  openId VARCHAR(64) UNIQUE NOT NULL COMMENT 'ID único para autenticação',
...
CREATE TABLE IF NOT EXISTS paymentMethods (
  ...
  isActive BOOLEAN DEFAULT TRUE COMMENT 'Se está ativa',
  createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'Data de criação',
  updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Data da última atualização',
...
CREATE TABLE IF NOT EXISTS quotes (
  id INT AUTO_INCREMENT PRIMARY KEY,
  number VARCHAR(50) UNIQUE NOT NULL COMMENT 'Número único do orçamento',
  userId INT NOT NULL COMMENT 'ID do usuário que criou',
  customerName VARCHAR(255) COMMENT 'Nome do cliente',
  customerPhone VARCHAR(20) COMMENT 'Telefone do cliente',
  ...
  totalAmount DECIMAL(12, 2) COMMENT 'Valor total do orçamento',
  createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'Data de criação',
  updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Data da última atualização',
...
CREATE TABLE IF NOT EXISTS quoteItems (
  ...
  productCode VARCHAR(50) NOT NULL COMMENT 'Código do produto (do Firebird)',
  productName VARCHAR(255) COMMENT 'Nome do produto',
  productBrand VARCHAR(100) COMMENT 'Marca do produto',
  quantity DECIMAL(10, 2) NOT NULL COMMENT 'Quantidade solicitada',
  unitPrice DECIMAL(12, 2) NOT NULL COMMENT 'Preço unitário',
  createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'Data de criação',
...
CREATE INDEX idx_quoteItems_quoteId_productCode ON quoteItems(quoteId, productCode);
...
LEFT JOIN paymentMethods pm ON q.paymentMethodId = pm.id;
...
-- Trigger: Atualizar totalAmount quando adicionar/remover itens
DELIMITER $$

CREATE TRIGGER trg_update_quote_total_after_insert
AFTER INSERT ON quoteItems
FOR EACH ROW
BEGIN
  UPDATE quotes 
  SET totalAmount = (
...
-- ─── PROCEDURES ÚTEIS ─────────────────────────────────────────────────────
-- Procedure: Gerar número único para orçamento
DELIMITER $$

CREATE PROCEDURE sp_generate_quote_number(OUT p_number VARCHAR(50))
BEGIN
  DECLARE v_year INT;
  DECLARE v_month INT;
  DECLARE v_count INT;
  ...
  FROM quotes 
  WHERE YEAR(createdAt) = v_year 
  AND MONTH(createdAt) = v_month;
  
  SET p_number = CONCAT(v_year, LPAD(v_month, 2, '0'), LPAD(v_count + 1, 5, '0'));
END$$

DELIMITER ;

-- ─── PERMISSÕES DE USUÁRIO ────────────────────────────────────────────────
-- Criar usuário segalla_user se não existir
CREATE USER IF NOT EXISTS 'segalla_user'@'localhost' IDENTIFIED BY 'senha_segura';
